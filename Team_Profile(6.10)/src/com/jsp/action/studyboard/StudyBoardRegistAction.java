...
public class StudyBoardRegistAction implements Action {
	...
	public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String url="/studyboard/regist_success";
		
		try {
		StudyBoardVO studyboard 
		= (StudyBoardVO)XSSHttpRequestParameterAdapter.execute(request, StudyBoardVO.class, true);
		
		studyboard.setS_content(request.getParameter("s_content"));
		
		studyboardService.regist(studyboard);
		}catch(Exception e) {
			e.printStackTrace();
			//url=null;
			throw e;
