			...
			<div class="card-body">
				<table class="table table-bordered text-center" >					
					...
					<c:forEach items="${noticeList }" var="notice">
						<tr style='font-size:0.85em;cursor:pointer;' onclick="OpenWindow('detail.do?from=list&i_no=${notice.i_no}','상세보기',800,700);">
							<td>${notice.i_no }</td>
							<td id="boardTitle" style="text-align:left;max-width: 100px; overflow: hidden; 
												white-space: nowrap; text-overflow: ellipsis;">
							${notice.i_title }
							</td>			
							<td data-target="notice-writer">${notice.i_writer}<td>							
								<fmt:formatDate value="${notice.i_regdate }" pattern="yyyy-MM-dd"/>
							</td>
							<td><span class="badge bg-red">${notice.i_viewcnt }</span></td>		
						</tr>
					</c:forEach>
				</table>				
			</div>
